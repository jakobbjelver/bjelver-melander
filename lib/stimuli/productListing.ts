...
interface ProductItem {
  ...
  interface ProductSummary {
    executiveText: string;        // top‑N extracted phrases
...
export function summarizeProducts(
    ...
    // A) Filter & text corpus
    const corpus = items
      .filter((p) => !p.irrelevant)
      .map((p) => [p.productName, p.description].join('. '));
  
    const tfidf = new TfIdf();
    corpus.forEach((doc) => tfidf.addDocument(doc));
  
    const sentences = corpus.flatMap((doc) =>
      doc.match(/[^\.!\?]+[\.!\?]+/g) || []
    );
    ...
    // B) Metadata
    const filtered = items.filter((p) => !p.irrelevant);
    const totalProducts = filtered.length;
    const inStockCount = filtered.filter((p) => p.inStock).length;
    const totalReviews = filtered.reduce((sum, p) => sum + p.reviewCount, 0);
    const avgRating =
      filtered.reduce((sum, p) => sum + p.rating, 0) / totalProducts || 0;
    ...
    const discountCount = filtered.filter((p) => !!p.discount).length;
  
    const brands = filtered.reduce((acc, p) => {
...
export const productListingTests: Question[] = [
    ...
    {
        id: "product_listing_comprehension",
