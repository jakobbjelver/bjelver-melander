...
interface TestQuestionFormProps {
  questions: Question[];
  participantId: string | null; // (null in try)
  testSlug: TestSlugs;
  action: TestResponseAction | null; // Server action to save responses (null in try)
}
...
export function TestQuestionForm({
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = action && await action(formData); // Call the server action

      if (!result) {
        toast.info("Cannot submit responses in test environment.")
        return
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 grid">
      ...
      {action && <Button type="submit" disabled={isLoading} size={'lg'} className='w-full max-w-md md:max-w-sm justify-self-center'>
