...
export default async function ContentPage({ params }: TestPageProps) {
  ...
  const content = await getTestContent(testSlug, participant.assignedSourceOrder, participant.assignedLength);
  if (!content) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-4 flex flex-col items-center">
      <TestContentDisplay testSlug={testSlug} contentData={content.contentData} />
